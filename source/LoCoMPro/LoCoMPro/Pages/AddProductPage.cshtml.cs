...
namespace LoCoMPro.Pages
{
    public class AddProductPageModel : PageModel
    {
        ...
        public IActionResult OnGetAutocompleteSuggestions(string field, string term, string provinceName, string cantonName, string storeName)
        {
            ...
            {
                var sourceStore  = _context.Stores
                                    .Include(s => s.Products)
                                    .FirstOrDefault(s => s.Name == storeName && s.ProvinciaName == provinceName && s.CantonName == cantonName);
                availableSuggestions = (sourceStore != null)? sourceStore.Products!.Select(p => p.Name).ToList() : new List<string>();
            }
